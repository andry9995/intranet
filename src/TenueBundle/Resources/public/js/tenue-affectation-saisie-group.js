...
{
    var url = Routing.generate('tenue_affectation_saisie_groupe', { json: 1 });
    $('.loader').show();
    ...
    $(document).find('.lot-panier-container').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (!ui.draggable.hasClass('dist')) {
                ...
                $.ajax({
                    url: Routing.generate('saisie_add_to_panier_groupe', {
                    ...
                    success: function (data) {
                        ...
                        if (data.erreur === false) {
                            ...
                            if (etape === 'OS_2')
                                ui.draggable.detach().addClass('dist lot_saisie_group2')
                                    .attr('data-date-panier-org', date_panier)
                                    .appendTo(lot_detail);
                            else if (etape === 'OS_1')
                                ui.draggable.detach().addClass('dist lot_saisie_group1')
                                    .attr('data-date-panier-org', date_panier)
                                    .appendTo(lot_detail);
                            else
                                    ...
                                    if ($(this).hasClass('lot_saisie1'))
                                    {
                                        $(this).find('.lot').each(function () {
                                            if ($(this).hasClass('hidden')) {
                                                ...
                                                $(this).addClass('lot_saisie_group1');
                                            }
                                            else {
                                                $(this).removeClass('lot_saisie_group2');
                                ...
                                $(classy).find('.lot-detail').each(function () {
                                    if ($(this).hasClass('lot_saisie2')) {
                                        $(this).find('.lot').each(function () {
                                            if ($(this).hasClass('hidden')) {
                                                $(this).removeClass('hidden');
                                                $(this).addClass('lot_saisie_group2');
                                            }
                                            else {
                                                $(this).removeClass('lot_saisie_group1');
                                                $(this).addClass('hidden');
                                            }
                                        });
