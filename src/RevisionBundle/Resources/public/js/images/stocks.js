...
$(function() {
    ...
    $(document).on('change', '#client-stocks', function(event) {
        ...
        $('#exercice-stocks').trigger('change');
    });

    /*Changement exercice*/
    $(document).on('change','#exercice-stocks',function(event) {
        event.preventDefault();
        event.stopPropagation();

        url = Routing.generate('app_dossiers_client',{
            client: $('#client-stocks').val(),
            exercice: $('#exercice-stocks').val()
        });
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#dossier-stocks").val('0').trigger('chosen:updated');
            }
        });
