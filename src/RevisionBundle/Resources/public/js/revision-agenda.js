$(function () {
    ...
    function showEvents(callback) {
        ...
        if (clients.length > 0) {
            ...
            var url = Routing.generate('revision_event_client', {
                clients: clients,
    ...
    function eventDrop(event, delta, revertFunc, jsEvent, ui, view) {
        if (event.type === 'local') {
            ...
            formData.append('client_id', event.client);
            formData.append('event_date', event.start.format());
            console.log(event, event.start.format());
            var url = Routing.generate('revision_event_date_update');
            fetch(url, {
                method: 'POST',
                credentials: 'include',
                body: formData
