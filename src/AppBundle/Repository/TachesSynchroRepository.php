...
class TachesSynchroRepository extends EntityRepository
{
    ...
    public function marquerFait(Dossier $dossier, TachesSynchro $tachesSynchro = null, $idGoogle = '', $nomTache = '',$status = 1,\DateTime $date = null, \DateTime $dateFait = null)
    {
        ...
        else
        {
            ...
            if ($config)
            {
                ...
                $nom = $nomTache.'+'.substr($dossier->getNom(),0,15);
     ...
     * @return TachesSynchro[]
     */
    public function getTachesSynchroForDossiers($dossiers, \DateTime $debut, \DateTime $fin, $isLegale = true, $isLibre = true)
    {
        $tachesSynchros = $this->createQueryBuilder('ts')
     ...
     * @return TachesSynchro[]
     */
    public function getTachesSynchroForRevisionClients($clients, \DateTime $debut, \DateTime $fin, $isLegale = true, $isLibre = true, $isFaite = true)
    {
        $tachesSynchros = $this->createQueryBuilder('ts')
            ->join('ts.dossier','d')
            ->join('d.site','s')
