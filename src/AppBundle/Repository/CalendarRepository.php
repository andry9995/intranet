...
class CalendarRepository extends EntityRepository
{
    ...
    public function getDossierTacheByTitle(Client $client,$title = '')
    {
        ...
        $dossierNom = trim($titleSpliters[1]);
        $tacheNom = trim($titleSpliters[0]);
    ...
    public function taches3Events($clients, \DateTime $periode)
    {
        ...
        foreach ($clients as $client)
        {
            ...
            if(array_key_exists($client->getId(), $tachesClients)){
     ...
     * @return array
     */
    public function taches3EventsNoUpdates($clients, $periode,
                                           $isLegale = true, $isLibre = true, $isFaite = true,
                                           $isScriptura = true, $isEc = false, $isCf = false)
    {
        $jourFeries = null;
        $idGoogleDefault = 'NONE';
        $codeScriptura = '#S: ';
        ...
        if(count($clients) > 0)
            $jourFeries = Functions::getJourFerie($clients[0], $periode);

        $tachesSynchros = $this->getEntityManager()->getRepository('AppBundle:TachesSynchro')
                ->getTachesSynchroForRevisionClients($clients,$debut,$fin,$isLegale,$isLibre,$isFaite);

        $taches = [];
        foreach ($tachesSynchros as $key => $ts)
        {
            if ($ts->getDate()->format('Y-m-d') <= '2019-08-01') continue;
            ...
            if(!in_array(($titre2.'*'.$date->format('ymd')), $dataTitre)){
                $taches[] = ModelAgenda::Tache3(
                    ...
                    $dossier->getSite()->getClient()->getTacheColor(),
                    '',
                    '',
                    $isDepasser
                );
    ...
    public function tachesSynchros(TachesEntity $tachesEntity = null,TachesDate $td = null, \DateTime $periode = null)
    {
        ...
        $additif = intval($tachesEntity->getJourAdditif());
        foreach ($taches as $tach)
        {
            ...
            foreach ($dates as $d)
            {
                ...
                {
                    if ($additif > 0)
                        $date->add(new \DateInterval('P' . $additif . 'D'));
                    else
                        $date->sub(new \DateInterval('P' . abs($additif) . 'D'));
                }
